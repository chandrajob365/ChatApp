...
exports.listen = function (server, session) {
      ...
      RoomDB.saveUserDetailList(groupMembersObj, () => {
        ...
        RoomDB.saveGroupUserList(groupMembersObj, () => {
          ...
          for (let newUser of groupMembersObj.users) {
            ...
            getGroupListForNewUser(newUser, (groupName) => {
              updateCurrentGroup(newUser, groupName)
            })
          }
  ...
  const getGroupListForNewUser = (newUser, cb) => {
    RoomDB.getUserDetailList(newUser, (groupList) => {
      if(groupList.length === 1)
      cb(groupList[0])
    })
  }
  const updateCurrentGroup = (newUser, groupName) => {
    console.log('<chat_server.js updateCurrentGroup > groupName = ', groupName, ' newUser = ', newUser)
    let activeSocketList = getActiveSockets(newUser)
    if (activeSocketList.length > 0) {
      for(let socketObj of activeSocketList) {
        socketObj.emit('setCurrentRoom', groupName)
        socketObj.emit('populateCurrentGroupName', groupName)
      }
    }
  }
